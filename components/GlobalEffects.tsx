...
export function GlobalEffects() {
  ...
  return (
    <>
      {/* 1. Cinematic Vignette — hidden on mobile to save GPU */}
      <div className="pointer-events-none fixed inset-0 z-[100] mix-blend-multiply opacity-60 hidden md:block">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_40%,#000_150%)]" />
      </div>

      {/* 2. Animated Film Grain/Noise — hidden on mobile to save GPU */}
      <div 
        className="pointer-events-none fixed inset-0 z-[110] opacity-[0.03] mix-blend-overlay hidden md:block"
        style={{
      ...
      {/* 3. Custom Cursor — only on desktop (pointer: fine) */}
      {isPointer && (
        <motion.div
          className={cn(
